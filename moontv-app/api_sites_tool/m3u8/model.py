...
class DefaultHTTPClient:
    ...
    def download(self, uri, timeout=None, headers={}, verify_ssl=True):
        ...
        base_uri = urllib.parse.urljoin(resource.geturl(), ".")
...
class BasePathMixin:
    ...
    def absolute_uri(self):
        ...
        ret = urllib.parse.urljoin(self.base_uri, self.uri)
        if self.base_uri:
            base_uri_parts = urllib.parse.urlsplit(self.base_uri)
            if (not base_uri_parts.scheme) and (not base_uri_parts.netloc):
                return ret

        if not urllib.parse.urlsplit(ret).scheme:
